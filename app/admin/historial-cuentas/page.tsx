...
export default function HistorialCuentasPage() {
  ...
  return (
    <div className="p-6 text-[#F5F1E8]">
      {/* Header con botón Salir (estilo Panel Administrativo) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-slate-800/80 p-4 rounded-xl border border-slate-700">
        ...
        <div className="flex flex-col">
          <label className="text-sm mb-1 text-[#D4AF37]">Tipo de movimiento</label>
          <select
            className="bg-slate-900 border border-slate-700 p-2 rounded-md text-sm"
      ...
      <div className="mt-6 overflow-x-auto border border-slate-700 rounded-xl bg-[#0F1B2E]">
        <table className="w-full text-xs md:text-sm">
          ...
          <tbody>
            {movimientos.map((m) => (
              <tr key={m.id_movimiento} className="odd:bg-slate-800 even:bg-slate-900">
                <td className="p-2">{new Date(m.fecha).toLocaleString("es-MX")}</td>
